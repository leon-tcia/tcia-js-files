...
function isEdgeBrowser() {
    // Check if the browser is Edge (both legacy and chromium-based)
    return navigator.userAgent.indexOf("Edge") > -1 || 
           (navigator.userAgent.indexOf("Edg") > -1 && navigator.userAgent.indexOf("Chrome") > -1);
}

function addStarBackground(scene, width, height, options = {}) {
    // Check if using Edge browser
    if (isEdgeBrowser()) {
        console.log("Edge browser detected, using static background");
        // Create a simple static starfield as fallback
    ...
    // Original star background code for other browsers
    const {
        starDensity = 35,
        minStarSize = 0.2,
        maxStarSize = 1.2,
...
function animate() {
    // Only animate if not using Edge and starBackground exists
    if (!isEdgeBrowser() && starBackground) {
        requestAnimationFrame(animate);
        renderer.render(scene, camera);
    }
}

// Modify the window resize handler
window.addEventListener('resize', debounce(() => {
    // Only handle Three.js resize if not using Edge
    if (!isEdgeBrowser()) {
        ...
        // Redraw static background for Edge
        addStarBackground(null, window.innerWidth, window.innerHeight);
    }
}, 250));

// Modify the Three.js setup section
let scene, camera, renderer, starBackground;

if (!isEdgeBrowser()) {
    // Initialize Three.js only for non-Edge browsers
    ...
    renderer = new THREE.WebGLRenderer({ 
        canvas: document.getElementById('star-background'), 
        alpha: true 
    });
    ...
    // Just create static background for Edge
    addStarBackground(null, window.innerWidth, window.innerHeight);
}
