...
function addStarBackground(scene, width, height, options = {}) {
    ...
    function twinkle() {
        ...
        gsap.to(colorValues, {
            ...
            ease: "power2.out",
            onUpdate: () => {
                colors[index * 3] = colorValues.r;
                colors[index * 3 + 1] = colorValues.g;
                colors[index * 3 + 2] = colorValues.b;
...
window.addEventListener('resize', debounce(() => {
    ...
    if (starBackground) {
        starBackground.geometry.dispose();
        starBackground.material.dispose();
...
function animateCards(timestamp) {
    ...
    if (timestamp - lastFrame < FRAME_LIMIT) {
        requestAnimationFrame(animateCards);
        return;
    }
    lastFrame = timestamp;
    ...
    if (!isHovering) {
        cards.forEach(card => {
            ...
            // 2. Optimize collision detection with spatial partitioning or reduce frequency
...
// Add this to your code to monitor memory usage
let memoryMonitorInterval;
if (window.performance && window.performance.memory) {
    memoryMonitorInterval = setInterval(() => {
        const memoryInfo = window.performance.memory;
        console.log(`Memory: ${Math.round(memoryInfo.usedJSHeapSize / 1048576)}MB / ${Math.round(memoryInfo.jsHeapSizeLimit / 1048576)}MB`);
        
        // Auto-reload if memory usage gets too high (80% of limit)
        if (memoryInfo.usedJSHeapSize > memoryInfo.jsHeapSizeLimit * 0.8) {
            console.warn('Memory usage too high, reloading page');
...
if (!checkWebGLSupport()) {
    console.warn('WebGL not fully supported. Using fallback.');
    // Disable Three.js animations and use a static background instead
    document.getElementById('star-background').style.display = 'none';
    document.body.style.backgroundImage = 'url(path/to/static/starfield.jpg)';
}
